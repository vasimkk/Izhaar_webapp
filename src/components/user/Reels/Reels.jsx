...
import {
  ...
  HiChatBubbleOvalLeft, HiOutlineChatBubbleOvalLeft
...
const Reels = () => {
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#050505] flex items-center justify-center overflow-hidden relative font-sans">
      <style>{`
      ...
      {/* Background Glows for Desktop View */}
      <div className="fixed inset-0 pointer-events-none hidden md:block">
        <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-pink-600/5 rounded-full blur-[120px] mix-blend-screen" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-purple-600/5 rounded-full blur-[120px] mix-blend-screen" />
      </div>

      {/* Main Responsive Container */}
      <div className="reel-container h-full w-full md:max-w-[420px] md:h-[92vh] md:rounded-[40px] bg-black relative overflow-hidden md:border-[6px] border-white/10 transition-all duration-500 shadow-2xl flex flex-col">

        {/* Premium Top Bar - Dedicated Space */}
        <div className="w-full h-[60px] md:h-[70px] bg-black/90 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-4 z-50 shrink-0">
          <button onClick={() => navigate(-1)} className="text-white p-2 active:scale-95 transition">
            <FaTimes size={24} className="opacity-70 hover:opacity-100" />
          </button>

          <div className="flex-1 mx-2 overflow-hidden relative h-6 flex items-center">
            <div className="whitespace-nowrap animate-marquee-top">
              <span className="text-white/90 font-bold text-[14px] tracking-[4px] uppercase italic">✨ Izhaar Reels ✨ Experience the Love ✨ Share the Magic ✨</span>
            </div>
          </div>

          <button onClick={toggleMute} className="text-white p-2 active:scale-95 transition">
            {muted ? <FaVolumeMute size={22} className="opacity-70" /> : <FaVolumeUp size={22} className="opacity-70" />}
          </button>
        </div>
          ...
          {reels.map((reel, index) => (
            <div key={reel.id} className="h-full w-full snap-start relative flex items-center justify-center overflow-hidden bg-black">
              ...
              <div className="absolute inset-0 pointer-events-none flex flex-col justify-end">
                ...
                {/* Bottom Info Bar (Left Side) - Adjusted for Instagram flat bar */}
                <div className="p-4 pb-20 bg-gradient-to-t from-black/80 via-black/20 to-transparent pointer-events-auto w-full z-20">
        ...
        <div className="absolute bottom-0 left-0 w-full h-[60px] md:h-[70px] bg-black/80 backdrop-blur-xl border-t border-white/5 flex items-center justify-around px-8 z-50 pointer-events-auto">
          {/* Home */}
          <button onClick={() => navigate('/user/dashboard')} className="flex items-center justify-center text-white/50 active:scale-90 transition-all">
            <HiOutlineHome size={28} />
          </button>

          {/* Create (Center +) */}
          <button onClick={() => setShowUpload(true)} className="w-[36px] h-[36px] rounded-[10px] border-2 border-white/90 flex items-center justify-center text-white active:scale-90 transition-all">
            <FaPlus size={18} />
          </button>

          {/* Profile */}
          <button onClick={() => navigate('/user/profile')} className="w-8 h-8 rounded-full border-2 border-white/40 overflow-hidden active:scale-90 transition-all">
            {userProfilePic ? (
              <img src={userProfilePic} alt="Profile" className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full bg-pink-600 flex items-center justify-center text-[10px] text-white">👤</div>
            )}
          </button>
        </div>
      </div>
